...
@RestController
@RequestMapping("/RoomLivingArea")
public class RoomLivingAreaRestController extends BaseRestController<RoomLivingArea> {
    private static Logger logger = LoggerFactory.getLogger(RoomLivingAreaRestController.class);

    @Autowired
    private RoomLivingAreaService service;

    /**
     * 为列表页面做数据准备
     */
    @Override
    protected void prepareQuery(@ApiIgnore @RequestHeader(WebHeaderCode.X_USER_TOKEN) String token, Page<RoomLivingArea> page, List<Filter> filters, List<Order> orders, HttpServletRequest request) {
        service.query(page, filters, orders);
    }

    @ApiOperation(value = "保存一条数据")
	@PostMapping(value = "/save")
	public Map<String, Object> save(@RequestBody RoomLivingArea saveInfo) {
